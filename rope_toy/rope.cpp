...
void display(void)
{
	...
	for (int i = 0; i < m_dynamicsWorld->getSoftBodyArray().size(); i++)
	{
        ...
        btSoftBodyHelpers::DrawFrame(psb, drawer);
...
void init(){
	...
	gluLookAt(50.0, 0.0, 50.0,  /* eye is at (0,0,5) */
...
int main(int argc, char* argv[])
{
    ...
    {
        // btSoftBody* rope = btSoftBodyHelpers::CreateRope(m_dynamicsWorld->getWorldInfo(), btVector3(-10, 5,0), btVector3(10, 5, 0), 16, 1+2);

		// rope->m_cfg.piterations = 4;
		// rope->m_materials[0]->m_kLST = 0.1;
		// rope->setTotalMass(1);

        // m_dynamicsWorld->addSoftBody(rope);

        m_dynamicsWorld->getWorldInfo().m_sparsesdf.RemoveReferences(0);
        struct Functors
        {
            static btSoftBody* CtorRope(const btVector3& p)
            {
                btSoftBody* psb = btSoftBodyHelpers::CreateRope(m_dynamicsWorld->getWorldInfo(), p, p + btVector3(10, 0, 0), 8, 1);
                psb->setTotalMass(50);
                psb->addForce(btVector3(0, -10, 0));
                m_dynamicsWorld->addSoftBody(psb);
...
btDeformableMassSpringForce* mass_spring = new btDeformableMassSpringForce(100,1, true);
	    ...
	    //btRigidBody* body = m_dynamicsWorld->createRigidBody(50, startTransform, new btBoxShape(btVector3(2, 6, 2)));
        btCollisionShape *shape = new btBoxShape(btVector3(2, 6, 2));
        double mass = 50;
        ...
        //m_dynamicsWorld->addForce(body, gravity);

	    btSoftBody* psb0 = Functors::CtorRope(btVector3(0, 8, -1));
        btSoftBody* psb1 = Functors::CtorRope(btVector3(0, 8, +1));
